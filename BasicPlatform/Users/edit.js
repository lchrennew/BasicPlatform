...
$(function () {
            ...
            tr.find('[data-control="roles"]').select2({
                ...
                initSelection: function (element, callback) {
                    var data = [],
                        ...
                        names = element.attr('data-names').split(',')
                    
                    for (var i = 0; i < ids.length; i++) {
                        data.push({ id: ids[i], text: names[i] })
                    }
                    callback(data)
                }
            })
    ...
    var f = new form('edit', function (f, data) {
        var apps = $('[name="apps"]'), username = $('#username').val(), i = 0
        if (apps.length) {
            var app = apps[i]
            function start(app) {
                i++
                ...
                if (a.length) {
                    ...
                    } else $(window).trigger($.extend($.Event('bind.ok'), { id: id, alias: tr.attr('data-alias') }))
                } else $(window).trigger('apps.setgroup.ok')
            }
    ...
    $(window).on('roles.add.ok', function (event) {
        ...
        if (tr.attr('data-alias'))
            $.getScript(tr.attr('data-url') + '?getgroups=1&a=' + encodeURIComponent(tr.attr('data-alias')))
        else $.getScript(tr.attr('data-url') + '?logout=silent')
    })
        .on('apps.usergroups.loaded', function (event) {
            var tr = $('[data-app="' + event.id + '"]'), s = tr.find('[data-control="group"] option').each(function () {
                if (this.value == event.groups[0]) this.selected = true
            })
            $.getScript(tr.attr('data-url') + '?logout=silent')
        })
        .on('bind.ok', function (event) { // 设置完成用户名触发此事件，开始设置用户组
            var tr = $('[data-app="' + event.id + '"]'), s = tr.find('[data-control="group"]')
            if (event.alias)
                $.getScript(tr.attr('data-url') + '?setgroups=1&a=' + encodeURIComponent(event.alias) + (s.val() ? '&g=' + encodeURIComponent(s.val()) : '')) // 触发apps.setgroup.ok，传回id
            else $(window).trigger('apps.setgroup.ok')
        })

    $('[name="apps"]:checked').change()
